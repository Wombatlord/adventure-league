...
class RosterView(arcade.View):
    ...
    def on_update(self, delta_time: float):
        ...
        if self.merc is None:
            ...
            self.entity_info.text = f"NAME: {self.merc.name.name_and_title} | LVL: {self.merc.fighter.level}  |  HP: {self.merc.fighter.hp}  |  ATK: {self.merc.fighter.power}  |  DEF: {self.merc.fighter.defence}"
