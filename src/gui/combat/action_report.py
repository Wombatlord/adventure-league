...
class ActionReport(arcade.Section):
    ...
    def after_action(self, event):
        dungeon: Dungeon = event[Events.VICTORY][Events.DUNGEON]

        for i, survivor in enumerate(eng.game_state.team.members):
            y = 20 * i
            survivor_name_text = arcade.Text(
                f"{survivor.name}",
                start_x=self.left + 100,
                start_y=self.height - 100 - y,
            )
            xp_text = arcade.Text(
                f"{dungeon.loot.awarded_xp_per_member}",
    ...
    def on_resize(self, width, height):
        ...
        self.title_text.position = self.bounds.center.x, self.height - 50

        labels = zip(self.survivor_labels, self.xp_labels)
        for i, label_pair in enumerate(labels):
            y = 20 * i
            label_pair[0].position = self.left + 100, self.height - 100 - y
            label_pair[1].position = self.width - 100, self.height - 100 - y
