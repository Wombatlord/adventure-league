...
def make_probability_func(sensitivity: float | int, accuracy: int = 0) -> Callable[[float | int], float]:
    return lambda delta: 1/2 + math.atan((delta + accuracy)/sensitivity)/math.pi

class AttackRules:
    @staticmethod
    def chance_to_hit(attacker: Fighter, target: Fighter) -> PercentChance:
        p = attacker.power
        d = target.defence
    ...
    @staticmethod
    def damage_amount(attacker: Fighter, target: Fighter) -> int:
        actual_damage = int(
            2 * attacker.power**2 / (attacker.power + target.fighter.defence)
        )
...
def projectile_attack(fighter: Fighter, target: Entity):
    result = {}
    if fighter.owner.is_dead:
        raise ValueError(f"{fighter.owner.name=}: I'm dead jim.")
    ...
    my_name = fighter.owner.name.name_and_title

    target_name = target.name.name_and_title

    successful_hit = fighter.roll_to_hit()
    result.update({"attack": fighter.owner})
    
    if successful_hit > target.fighter.defence:
        actual_damage = int(
            2 * fighter.power**2 / (fighter.power + target.fighter.defence)
        )
