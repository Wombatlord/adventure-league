...
def bad_syllables_likely() -> str:
    ...
    allowed = ["these", "are", "okay", "and", "should", "comprise", "the", "final", "word"]
    if flip < 4:
        return "disallow"
...
class SyllablePatches:
    @classmethod
    def syllables_with_recursion_printout(cls, min_syls=1, max_syls=3, syl_func: Callable | None=None):
        """
        ...
        """
        if not syl_func:
            raise ValueError(f"No syllable generation function. Expected Callable, got {syl_func=}")
        ...
        for syl in word:
            if syl := inoffensive(syl):
                ...
                word = cls.syllables_with_recursion_printout(syl_func=bad_syllables_likely)
