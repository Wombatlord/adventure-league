...
class VoxelGrid:
    ...
    def next_maybe(
        self, start: Node, can_visit: Callable[[Voxel], bool]
...
class JonnyCash(Visitor):
    ...
    _current_contour: Contour | None
    _last_node: Node | None
    todo_list: dict[str, list[Voxel]]

    def __init__(self, regionnaire: Regionaire, grid):
        self.grid = grid
        self.contours = []
    ...
    def visit(self, vox: Voxel):
        self._last_node = vox.location

        if self._current_contour is None:
            labels: set[str] = {
                v.tile.char
                for v in vox.get_adj_maybe(lambda v: v and not v.tile.is_boundary())
            }
            try:
                self._current_contour = Contour.from_start(vox.location, labels)
            except ValueError:
                return
        self._current_contour.nodes.add(vox.location)
    ...
    def can_visit(self, vox: Voxel) -> bool:
        if not self.can_start(vox):
            return False

        if self._last_node is None:
            return True

        displacement = Node(*(vox.location - self._last_node)[:2])
        ...
        return vox.tile.char in allowed[displacement]

    def end(self):
        for contour in self.contours:
            for node in contour.nodes:
                vox = self.grid[node]
                lowest = min(*contour.heights)
                vox.tile = f"{lowest}"


class ContourResolver(Visitor):
    def __init__(self, jonny: JonnyCash):
        self.contours = jonny.contours
...
def main():
    ...
    print(grid)
    print()
    jonny = JonnyCash(regionaire, grid)
